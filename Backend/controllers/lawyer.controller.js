...
export const createNewLawyer = async (lawyerData) => {
  ...
  try {
    ...
    const isLawyer = await getLawyerByEmailAndMobile(
      lawyerData.emailId,
      lawyerData.mobileNumber,
    );
    if (isLawyer) {
...
export const uploadLawyerPic = async (image) => {
  const error = { error: "Error while uploading image!" };
  try {
    console.log(image);
    return { image: image };
  } catch (err) {
